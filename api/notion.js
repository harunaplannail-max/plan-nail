...
// req.bodyが自動で解析されない環境でも確実に読み取れるようにするフォールバック付きヘルパー
...
module.exports = async (req, res) => {
  ...
  try {
    if (type === 'feedback' && req.method === 'POST') {
      ...
      // Notionのタイトルは最大2000文字程度が目安のため、念のため長すぎる場合は切り詰める
    ...
    if (type === 'trends') {
      ...
      res.json(r.results.map(page => {
        ...
        return {
          ...
          images: images,
          mood: getProp(p,'ムード'),
          moodCoord: getProp(p,'ムード座標') || null, // "x,y" 形式。x:0シンプル〜100ギャル、y:0ポップ〜100ダーク
          created: page.created_time,
          releaseDate: getProp(p,'発売日') || null,
          // カンマ区切りのテキストをそのまま配列にして返す。実際のリンクはフロント側で名前照合して行う
