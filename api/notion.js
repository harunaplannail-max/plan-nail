...
// URLを貼るだけでサムネイルを自動取得するための補助関数。
// YouTubeは動画IDから直接サムネイルURLを組み立てられるので確実&高速(通信不要)。
// それ以外(note.com等)はページ先頭のOG画像(og:image)メタタグを軽く読みに行く。
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      const req = https.get({
        ...
        res.on('data', chunk => {
          raw += chunk;
          // og:imageは通常<head>内、ページ先頭付近にあるので大きく読み過ぎないうちに打ち切る
          if (raw.length > 150000) res.destroy();
        });
        res.on('end', () => {
          const m = raw.match(/<meta[^>]+property=["']og:image["'][^>]*content=["']([^"']+)["']/i)
  ...
  try {
    ...
    if (type === 'media') {
      ...
      const items = await Promise.all(filtered.map(async page => {
        ...
        if (!thumbnail && url) {
          // サムネイル欄が空ならURLから自動取得を試みる(取れなくても致命的ではないので握りつぶす)
          thumbnail = youtubeThumbnail(url) || (await fetchOgImage(url).catch(() => null)) || '';
        }
        return {
          id: page.id,
          title: getProp(p, 'タイトル'),
          thumbnail,
          url,
